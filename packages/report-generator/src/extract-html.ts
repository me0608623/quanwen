/**
 * HTML extraction — vendored from nexu-io/html-anything extract-html.ts
 *
 * Pulls the actual HTML document out of an AI agent's possibly chatty response.
 * Agents sometimes wrap output in ```html ... ``` fences or prepend explanation.
 */

/**
 * Extract a complete HTML document from a potentially noisy AI response.
 * Handles: fenced code blocks, DOCTYPE declarations, raw HTML, and plain text fallback.
 */
export function extractHtml(streamed: string): string {
  ...
  // 5. Wrap whatever we got in a minimal scaffold
...
/**
 * For previews while the stream is still arriving — ensure closing tags
 * so the iframe can render incrementally.
 */
