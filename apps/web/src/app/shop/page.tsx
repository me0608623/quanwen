...
import { useShopItems, useRedeem, CATEGORY_LABEL, CATEGORY_BADGE, ShopItem } from '@/hooks/use-shop';
...
export default function ShopPage() {
  ...
  const handleRedeem = async (item: ShopItem) => {
    try {
      ...
      setConfirming(null);
      alert(`✅ 兌換成功！PIN 序號已寄到您的通知中心，可在「我的兌換」查看`);
    } catch (err: unknown) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      ...
      <div className="rounded-xl border border-amber-300/40 bg-gradient-to-br from-amber-50 to-yellow-50 p-4">
        <p className="text-xs text-amber-700 uppercase tracking-wide">目前積分</p>
        ...
        <p className="mt-1 text-[11px] text-amber-600">完成問卷可累積積分，到此兌換禮券</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-baseline gap-2 pt-1">
                <span className="text-2xl font-bold text-amber-700 tabular-nums">{item.costPoints}</span>
