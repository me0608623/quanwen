...
/**
 * Shuffle question order for display. Similar to shuffleOptions but works on
 * question indices/items. The seed typically incorporates the response ID so
 * each respondent gets a unique but reproducible order.
 */
export function shuffleQuestions<T>(items: T[], mode: ShuffleOption, seed: string): T[] {
  // Same logic applies for questions
  return shuffleOptions(items, mode, seed);
}

/**
 * Generate a short random seed (8 hex chars) for a new response.
 * This is stored in survey_responses.randomizationSeed for reproducibility.
 */
export function generateSeed(): string {
  const bytes = new Uint8Array(4);
  // In Node.js crypto is available globally in tests
  try {
    const { randomBytes } = require('crypto');
    const buf = randomBytes(4);
    return buf.toString('hex');
  } catch {
    // Fallback for browser / non-Node environments
